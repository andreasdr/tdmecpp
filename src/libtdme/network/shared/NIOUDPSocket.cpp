...
ssize_t NIOUDPSocket::read(std::string& from, unsigned int& port, void* buf, const size_t bytes) throw (NIOIOException) {
	// read from socket
...
ssize_t NIOUDPSocket::write(const std::string& to, const unsigned int port, void* buf, const size_t bytes) throw (NIOIOException) {
	// set up receiver
	struct sockaddr_in sin;
	socklen_t sin_len = sizeof(sin);
...
void NIOUDPSocket::createServerSocket(NIOUDPSocket& socket, const std::string& ip, const unsigned int port) throw (NIOSocketException) {
	// create socket
	...
	try {
		...
		if (setsockopt(socket.descriptor, SOL_SOCKET, SO_REUSEOPTION, (void*)&flag, sizeof(flag)) == -1) {
			std::string msg = "Could not set reuse port on socket: ";
			msg+= strerror(errno);
			throw NIOSocketException(msg);
		}
