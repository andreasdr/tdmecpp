/**
 * @version $Id: d138715cc4be01ad050e71004ab8d30548d30bf0 $
 */
...
NIOUDPServerIOThread::NIOUDPServerIOThread(const unsigned int id, NIOUDPServer *server, const unsigned int maxCCU) :
	TDMEThreading::Thread("nioudpserveriothread"),
	id(id),
	server(server),
	maxCCU(maxCCU),
...
void NIOUDPServerIOThread::run() {
	Logger::log(
		Logger::LEVEL_INFO,
		"NIOUDPServerIOThread[%u]::run(): start",
		id
	);
	...
	try {
		...
		// do event loop
		uint64_t lastMessageQueueAckTime = Time::getTimestamp();
				...
				if (hasReadInterest) {
					...
					while ((bytesReceived = socket.read(ip, port, (void*)message, sizeof(message))) > 0) {
						...
						try {
							...
							Logger::log(
								Logger::LEVEL_INFO,
								"NIOUDPServerIOThread[%u]::run(): %s : %s",
								id,
	...
	//
		...
		Logger::log(
			Logger::LEVEL_INFO,
			"NIOUDPServerIOThread[%i]::run(): %s : %s",
			id,
			TDMEGlobal::RTTI::demangle(typeid(exception).name()).c_str(),
			exception.what()
		);
	...
	Logger::log(
		Logger::LEVEL_INFO,
		"NIOUDPServerIOThread[%u]::run(): done",
		id
	);
}
