...
namespace TDMEJson {
	...
	Value &Value::operator[](Array::size_type index) throw (JsonException) {
		if (type != ARRAY) {
	...
	const Array &Value::getArray() const throw (JsonException) {
		if (type == ARRAY) {
	...
	void Value::readNumber(std::istream &input, TDMEJson::Value &result) throw (JsonException) {
